...
const useStyles = makeStyles(theme => ({
  ...
  export default function ReferenceIndicator() {
   ...
   const createIndicatorListForUI = function(indicatorsDataOCL) {
    ...
    var queryIndicators = "https://api." + domain + "/orgs/" + org + "/collections/MER_REFERENCE_INDICATORS_FY" + values.fiscal.trim().substring(2,4) + "/concepts/?limit=999&verbose=true";
    ...
    const loadDataElementsDataByIndicator = async (indicatorID)=> {   
      ...
      try {       
        ...
        if (!jsonData) {
          ...
          setDELoading(false);
          throw new Error(
            `Warning data element data is empty from OCL  ` + indicatorID
          );
        }
        ...
        <div key={dataElement.external_id}>        
              <Grid container alignItems="center"  className={classes.expansionPanelSummary} style={{padding: '4px'}}>       
                <Grid item  xs={12} md={12}>         
                  ...
                  <Grid item xs={12} md={5}  className={classes.chip}>                                                            
